...
footer: "[https://dicook.github.io/ResBaz/slides.html](https://dicook.github.io/ResBaz/slides.html)"
---
...
#| fig-height: 3
#| out-width: 100%
#| fig-alt: "Plot of residuals against fitted values shows a slight non-linear pattern. Two outliers, high residual but low fitted value affect the reading of the nonlinear pattern."
...
#| fig-width: 6
#| fig-align: center
#| fig-alt: "Scatterplot of first two linear discriminants, with points coloured by wasp species, F, G, Q, W. F and G are well-separated clusters but Q and W are adjoining."
data(wasps)
...
:::: {.columns}
::: {.column}
![](images/thyroid-choro-1.png){alt="Choropleth map of thyroid cancer across Australian LGAs. High incidence appears to be around Brisbane and west of Sydney city."}

:::
::: {.column}
![](images/thyroid-hex-1.png){alt="Hexagon tile map of thyroid cancer across Australian LGAs. High incidence is seen across the Brisbane, Sydney and Perth areas. Some LGAs in Melbourne have higher values too."}
:::
::::
...
#| label: aus-tb
#| fig-height: 2.5
#| fig-alt: "Scatterplot of TB incidence (ranging from 900-1600) against years 1997-2021. It has 2 layers. Layer 1 is a set of 25 big solid circle points of which about 100% can be seen. Layer 2 is a lowess smoothed curve. Layer 2 has colour set to vivid red. There is a roughly linearly increasing pattern. "
ggplot(tb_aus, 
       aes(x=year, 
           y=c_newinc)) + 
  geom_point() +
...
#| fig-height: 3.5
#| out-width: 80%
#| fig-alt: "Density plots of flipper length ranging from 170-230 and bill length ranging from 30-60. Points are coloured by species. The three species form clusters, but they are not separated. Adelie is at low left, Gentoo is upper right, and Chinstrap is middle and mid-to-upper positions. "
ggplot(penguins, 
       aes(x=flipper_length_mm, 
           y=bill_length_mm, 
           color=species)) + 
...
#| fig-alt: "The chart is comprised of 14 panels containing sub-charts, arranged in a grid. The panel rows represent different values of sex. The panel columns represent different values of age. There is a legend indicating colour is used to show sex, with 2 levels: females shown as deep pink colour and males shown as light olive colour. Each sub-chart has 2 layers. Layer 1 of is a set of 9 big solid circle points, and layer 2 is a lowess smoothed curve."
ggplot(tb_aus_sa, 
       aes(x=year, 
           y=count, 
           colour=sex)) + 
...
#| eval: false
#| echo: true
#| fig-alt: "Tiled spatial map showing average January 1998 temperatures in Kelvin over Central America, the lower few states of the USA and the north part of South America. Country boundaries are overlaid. Warmer temperatures are seen in ocean regions. Land masses are cooler, particularly north America and the mountain region of South America."
ggplot() + 
  geom_tile(DATA, 
    aes(x=LONGITUDE, 
        y=LATITUDE, 
...
#| fig-height: 8
#| out-width: 60%
#| fig-alt: "Top plot is the same as that plotted on slide 33. Bottom plot has points coloured on permuted class, and the clusters so not match the colors."
p1 <- ggplot(penguins, 
       aes(x=flipper_length_mm, 
           y=bill_length_mm, 
           color=species)) + 
